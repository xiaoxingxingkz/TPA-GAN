...
# initial setup
MODEL_PATH = './Classifier_save'   #'''!!!!!!!!!!!!!!!!!!manually!!!!!!!!!!!!!!!!!!!!!!!!'''
...
# load optimazer
t_optimizer = optim.Adam(T.parameters(), lr=LR, weight_decay=1e-2) 
    ...
    for iteration in range(EPOCH):
        ...
        for train_data in data_loader_train:
            ...
            for itera_f in range(iter_t):
                ...
                #input
                classi_r = T(mri_images, pet_images)
        ...
        for train_s in range(1):
            ...
            for val_trian_data in data_loader_valid:
                ...
                #input
                result_c = T(mri_images, pet_images)
                ...
                # if result_c[0].data.cpu().item() > 0.5:
                #     PREDICTED_ = 1
                # else:
                #     PREDICTED_ = 0
        ...
        #save model
        torch.save(T.state_dict(), os.path.join(MODEL_PATH, '{}_TLoss{}_TrainACC{}_TestACC{}_TestSEN{}_TestSPE{}_TestAUC{}_F1S{}_T.pth'.format(
            iteration + 1, 
